...
session_start();
if(!isset($_SESSION['token']) || (int)$_SESSION['level'] < 9 ||$_SESSION['locked'] == true) {
	header('location: /');
	die;
}
...
$(function(){
	...
	function renderFileRow(data) {
		var $link = $('<a class="name" />')
			.attr('href', data.is_dir ? '#' + data.path : './'+data.path).css('color', '#2196F3')
			.text(data.name);
		...
		var $html = $('<tr />')
			...
			.append( $('<td/>').attr('data-sort',data.mtime).text(formatTimestamp(data.mtime)) ).css('color', '#2196F3')
	...
	function renderBreadcrumbs(path) {
		var base = "",
			$html = $('<div/>').append( $('<a href=# style="color:#2196F3">Home</a></div>') );
		$.each(path.split('/'),function(k,v){
			if(v) {
				$html.append( $('<span/>').text(' ▸ ') )
					.append( $('<a/>').attr('href','#'+base+v).css('color', '#2196F3').text(v) );
				base += v + '/';
			}
		});
		return $html;
