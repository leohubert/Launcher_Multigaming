            ...
            if ($getSettings->rowCount() != 0) {
                if ($support['started_by'] == $myID)
                {
                    if ($getSettings = $database->prepare('SELECT * FROM `support_conversation` WHERE support_id=:id ORDER BY `support_conversation`.`send_at` ASC')) {
                        ...
                        $user->execute(array('id' => $support['assign_to']));
                        $userName = $user->fetch();
                        if ($user->rowCount() != 0) {
                            $result['support_admin_name'] = $userName['username'];
                            $result['support_admin_email'] = $userName['email'];
                            $result['support_admin_level'] = $userName['level'];
                            $result['support_admin_picture'] = $userName['picture'];
                        } else {
                ...
                {
                    $result['status'] = 44;
                    $result['message'] = "You don't have right to get this support !";
                }
            }
            else
            {
