...
if (isset($_POST['token']) && isset($_POST['id']) && is_numeric($_POST['id']))
{
    ...
    if ($checkUser->rowCount() != 0 && $userLevel = $database->prepare('SELECT `level`,`banned` FROM users WHERE id = :id'))
    {
        ...
        $myID = $res['user_id'];
        $res = $userLevel->fetch();
        if ($userLevel->rowCount() != 0 && (int)$res['level'] >= 9  && $res['banned'] != 1)
        {
            $getSettings = $database->prepare('SELECT id FROM `news` WHERE id=:id');
            $getSettings->execute(array('id' => $id));
            if ($getSettings->rowCount() != 0) {
                if ($getSettings = $database->prepare('DELETE FROM `news` WHERE id = :id')) {
                    $getSettings->execute(array('id' => $id));
                    $result['status'] = 42;
                    $result['message'] = "News removed";
                }
            }
            else
            {
